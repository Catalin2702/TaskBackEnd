...
std::optional<Task> TaskService::createTask(const std::string& title, const int categoryId, const TaskStatus status, const std::string& description) const {
	const auto results = session->query(Task{title, categoryId, status, description}).insert();
	session->commit();
	if (results.has_value())
		return results->front();
	return std::nullopt;
}
std::optional<Task> TaskService::updateTask(const unsigned long id, const std::optional<std::string>& title, const std::optional<int> categoryId, const std::optional<TaskStatus> status, const std::optional<std::string>& description) const {
	Task task{};
	if (title.has_value())
		task.title = title.value();
	if (categoryId.has_value())
		task.categoryId = categoryId.value();
	if (status.has_value())
		task.status = status.value();
	if (description.has_value())
		task.description = description.value();
	task.updated = std::chrono::system_clock::now();
...
std::vector<unsigned long> TaskService::deleteCategoriesTasks(const std::vector<unsigned long>& categoryIds) const {
	const Task task{};
	std::vector<int> unsignedIds;
	unsignedIds.reserve(categoryIds.size());
	for (const auto& id : categoryIds)
		unsignedIds.push_back(static_cast<int>(id));
	const auto results = session->query(task).filter(task.categoryId.in(unsignedIds)).remove();
	session->commit();
	return results;
}
std::vector<unsigned long> TaskService::deleteCategoryTasks(const unsigned long categoryId) const {
	const Task task{};
	const auto results = session->query(task).filter(task.categoryId == static_cast<int>(categoryId)).remove();
	session->commit();
	return results;
}
std::vector<unsigned long> TaskService::deleteUsersTasks(const std::vector<unsigned long>& userIds) const {
	if (userIds.empty())
		return {0};
	const Category category{};
	std::vector<int> u_ids;
	u_ids.reserve(userIds.size());
	...
	for (const auto& cat : categories.value())
		c_ids.push_back(static_cast<int>(cat.id.getValue()));
	const auto results = session->query(task).filter(task.categoryId.in(c_ids)).remove();
	session->commit();
	return results;
}
std::vector<unsigned long> TaskService::deleteUserTasks(const unsigned long userId) const {
	const Category category{};
	const auto categories = session->query(category).filter(category.userId == static_cast<int>(userId)).all();
	if (not categories.has_value())
		return {0};
